...
#include "StdAfx.h"
#include "AES.h"
#include "Global.h" //只是增加了一个交换函数，因为std::swap<byte>(a,b) //编译不过去，有点bug 其实内容为‘
/*
...
{    //    密钥扩展
	...
	for (int roundIndex = 1; roundIndex < N_ROUND; ++roundIndex)
	{
		...
		exmath::swap<byte>(rotWord[0], rotWord[1]);
		exmath::swap<byte>(rotWord[1], rotWord[2]);
		exmath::swap<byte>(rotWord[2], rotWord[3]);
		for (int i = 0; i < 4; ++i)
		{
...
{    //    行变换
	 ...
	 // Do nothing.
	 //state第二行循环左移一个字节
	exmath::swap<byte>(state[4], state[5]);
	exmath::swap<byte>(state[5], state[6]);
	exmath::swap<byte>(state[6], state[7]);
	//state第三行循环左移两个字节
	exmath::swap<byte>(state[8], state[10]);
	exmath::swap<byte>(state[9], state[11]);
	//state第三行循环左移三个字节
	exmath::swap<byte>(state[14], state[15]);
	exmath::swap<byte>(state[13], state[14]);
	exmath::swap<byte>(state[12], state[13]);
}
